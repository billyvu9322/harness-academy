...
describe('scoreQuestion', () => {
  it('passes when judge passes, answer is cited and expected doc + keywords match', () => {
    const r = scoreQuestion(
      grounded,
      { answer: 'Feature list là một primitive nền tảng.', citations: [cite()] },
      goodVerdict,
    );
  ...
  it('fails when the judge fails', () => {
    const r = scoreQuestion(
      grounded,
      { answer: 'Feature list là một primitive.', citations: [cite()] },
  ...
  it('fails a grounded question with no citations', () => {
    const r = scoreQuestion(
      grounded,
      { answer: 'Feature list là một primitive.', citations: [] },
      goodVerdict,
    );
  ...
  it('fails when the expected doc was not cited', () => {
    const r = scoreQuestion(
      grounded,
      { answer: 'Feature list là một primitive.', citations: [cite({ sourcePath: 'academy/content/lectures/99-other.md', route: '/lectures/99-other', title: 'Other' })] },
      goodVerdict,
    );
  ...
  it('for an uncertain question, grounding is not required', () => {
    ...
    const r = scoreQuestion(
      uncertain,
      { answer: 'Tài liệu không đề cập tỷ giá này.', citations: [] },
      goodVerdict,
    );
    expect(r.cited).toBe(false);
    expect(r.pass).toBe(true);
...
describe('aggregate', () => {
  it('computes pass rate and average score', () => {
    const agg = aggregate([
      { id: 'a', judgeScore: 5, judgePass: true, cited: true, expectedDocCited: true, keywordHits: 1, keywordTotal: 1, pass: true },
      { id: 'b', judgeScore: 3, judgePass: false, cited: true, expectedDocCited: true, keywordHits: 0, keywordTotal: 1, pass: false },
    ]);
  ...
  it('is safe on an empty set', () => {
    const agg = aggregate([]);
    expect(agg).toEqual({ total: 0, passed: 0, passRate: 0, avgScore: 0 });
  });
});

describe('meetsBaseline', () => {
  it('passes when both thresholds are met', () => {
    const v = meetsBaseline({ total: 6, passed: 5, passRate: 0.83, avgScore: 4.2 });
    expect(v.ok).toBe(true);
    expect(v.reasons).toEqual([]);
  });

  it('fails and reports each unmet threshold', () => {
    const v = meetsBaseline({ total: 6, passed: 2, passRate: 0.33, avgScore: 2.1 });
