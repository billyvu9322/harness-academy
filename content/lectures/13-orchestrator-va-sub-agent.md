...
> **Orchestrator** = agent "chỉ huy". Không tự code. Phân việc cho sub-agent.
...
### ❌ Orchestrator tự code

Orchestrator phải **delegate**. Tự code = mất context isolation = vô nghĩa.
...
| OpenCode | task subprocess | inline + provider switch |

Claude Code chuẩn nhất — có **typed agent** (researcher, reviewer, debugger...) trong `.claude/agents/`.
...
- Orchestrator điều phối, không code
