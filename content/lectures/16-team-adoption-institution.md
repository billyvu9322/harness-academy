...
### 1. File hướng dẫn theo lớp (Layered CLAUDE.md)

4 lớp file load theo thứ tự ưu tiên + vị trí:

```
Cấp tổ chức (org):  /etc/claude-code/CLAUDE.md
Cấp người dùng:     ~/.claude/CLAUDE.md
Cấp project:        <repo>/CLAUDE.md
Cấp module:         <repo>/.claude/rules/*.md
Cấp local:          CLAUDE.local.md (không commit, dành cho override cá nhân)
```
...
→ Cùng project, mọi người có cùng quy tắc quyền. Không "tôi tin agent hơn bạn nên cho phép nhiều hơn".
...
→ Đúng: project CLAUDE.md là nguồn duy nhất. Cấp user chỉ chứa preference cá nhân không ảnh hưởng output (theme, alias).
...
|------|---------|--------|
| 1 | Tập trung quy tắc chung | Viết `CLAUDE.md` cấp project, commit vô repo |
...
| Vai trò | Trách nhiệm |
|---------|-------------|
| **Tech Lead** | Định nghĩa CLAUDE.md cấp project, approval boundary, DoD |
...
**Nền tảng (L01 → L12)** — vì sao agent fail, harness là gì, 7 layer cốt lõi, kỷ luật vận hành.
