...
## Cách dùng bài này với harness template có sẵn

Repo đã có template hoàn chỉnh tại `templates/automation-test-harness-experimental/`.

Bạn có 2 cách học:

| Cách | Khi nào dùng | Bạn sẽ làm gì |
|------|--------------|---------------|
| **Build từ đầu** | Muốn hiểu từng mảnh harness | Làm từ Bước 0 tới Bước 11 trong bài này |
| **Dùng template sẵn** | Muốn chạy workflow nhanh hoặc xem bản chuẩn | Copy `templates/automation-test-harness-experimental/` thành repo mới, rồi bắt đầu từ phần "Quick Start" trong template README |

Template không thay thế bài học. Template là **đáp án tham chiếu**: sau khi làm từng bước, đối chiếu với template để thấy một automation test harness đầy đủ gồm `AGENTS.md`, `CLAUDE.md`, `docs/harness/`, `.claude/agents/`, `.claude/skills/`, `specs/`, `tests/`, `reports/`, và `scripts/bin/harness-cli.mjs`.

Muốn chạy nhanh từ template:

```bash
cp -R templates/automation-test-harness-experimental my-test-harness
cd my-test-harness
npm install
npx playwright install
npm run harness:intake -- --type user_story --summary "Login valid user" --lane normal
```

Nếu dùng PowerShell:

```powershell
Copy-Item -Recurse templates\automation-test-harness-experimental my-test-harness
Set-Location my-test-harness
npm install
npx playwright install
npm run harness:intake -- --type user_story --summary "Login valid user" --lane normal
```

Sau đó mở `my-test-harness/README.md`, làm flow `test-planner -> test-generator -> test-healer`. Template cố ý **chưa có generated test**; chỉ chạy `npm run test:auth` sau khi agent đã tạo `tests/auth/login-valid.spec.ts`.

Nếu bạn đang dạy workshop, cách tốt nhất là:

1. Dùng bài này để giải thích từng quyết định harness.
2. Mở template để xem bản đã lắp ráp.
3. Yêu cầu học viên sửa template cho app thật của họ: đổi product docs, risk lane, test matrix, selectors, fixtures, validation command.

## Bước 0 — Setup repo từ đầu

Nếu bạn chọn dùng template sẵn, bỏ qua Bước 0-8 và mở `templates/automation-test-harness-experimental/README.md`. Nếu muốn hiểu từng layer, làm các bước dưới đây.
