...
Hoặc: chỉ còn intentional changes, staged đúng, commit message rõ, blocker/next step đã ghi lại.
...
2. Confirm with user: commit / discard / stash. Không tự xoá thay đổi chưa rõ nguồn gốc.
...
Một số harness có tool worktree riêng; nếu không, dùng `git worktree` hoặc branch riêng theo workflow team.
