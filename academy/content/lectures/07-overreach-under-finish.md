...
Model có xu hướng tối ưu theo "làm code tốt hơn" thay vì "làm đúng phạm vi". Khi thấy code không sạch, agent dễ sửa thêm dù bugfix không yêu cầu.

### Under-finish

Verify tốn thời gian và tool call. Nếu harness không bắt buộc, agent dễ chuyển từ "đã viết code" sang "đã xong".
...
Nếu touch file không liên quan → STOP, justify trước khi tiếp tục. Không tự revert nếu có thể là thay đổi của người khác.
```
...
description: Bắt buộc chạy trước khi claim task xong.
---
...
Memory file rule:
```
Không claim done nếu chưa paste output kiểm chứng.
Nếu không chạy được test, nói rõ lý do, không claim done.
```
