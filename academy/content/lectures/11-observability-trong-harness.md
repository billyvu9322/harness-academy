...
Quan trọng: trace không chỉ để debug sau lỗi. Trace là dữ liệu để đánh giá **trajectory**: agent có đi đường an toàn, hiệu quả, recover được không.
...
Lệnh nào đã chạy, exit code, artifact nào sinh ra, feature item nào được chứng minh.
...
- **Premature completion rate**: số lần agent claim done trước khi đủ evidence
- **Human intervention count**: bao nhiêu lần user phải cứu session

→ Improve harness dựa trên số, không cảm tính.

## Trace-driven reliability loop

Một harness tốt học từ trace:
...
Nếu trace cho thấy agent fail vì thiếu command test, sửa `AGENTS.md`. Nếu fail vì tool output mơ hồ, sửa tool schema. Nếu fail vì không có E2E, sửa test matrix.
...
- Trace dùng để score cả output lẫn đường đi
