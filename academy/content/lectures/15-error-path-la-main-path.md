...
- Test đường lỗi như first-class case, không chỉ test happy path

Mỗi lỗi nên trả về observation có ích:

```json
{
  "status": "error",
  "summary": "POST /api/reset-password returned 500",
  "artifacts": ["test-results/reset-password.trace.zip"],
  "next_actions": ["Check EMAIL_PROVIDER_API_KEY mock config"],
  "recovery_hint": "Retry after mock email provider is configured; stop if migration state changed"
}
```

Agent recover tốt khi output lỗi nói rõ chuyện gì xảy ra, artifact ở đâu, bước an toàn tiếp theo là gì.
...
- Error output phải có status, summary, artifact, next action, recovery hint
