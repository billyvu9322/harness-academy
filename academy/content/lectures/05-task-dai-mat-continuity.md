...
### 2. Todo list trong session

Todo list giúp agent bám việc trong session hiện tại. Nhưng đừng xem nó là state bền vững cho nhiều ngày hoặc nhiều tool. State quan trọng phải ghi vào repo: plan file, PR description, commit, hoặc trace.
...
2. Nếu team cho phép WIP commit: commit với prefix `wip:`. Nếu không, ghi rõ dirty files và next step vào plan.
...
- Plan file
- Checkpoint đều đặn: commit WIP nếu workflow cho phép, hoặc plan update nếu chưa được commit
- Todo/session task cập nhật realtime
- Cuối session: summary "đã xong / còn lại / blocker"

## Điểm chính

- Compaction là tất yếu. State phải sống ngoài context.
- Plan file + git/PR + trace = state bền vững. Todo chỉ hỗ trợ điều phối trong session.
